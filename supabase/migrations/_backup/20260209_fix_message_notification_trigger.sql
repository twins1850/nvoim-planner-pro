-- Fix message notification trigger to use correct column names
-- 2026-02-09
-- Issue: Trigger was using p.name and sp.name which don't exist
-- Fix: Change to p.full_name and sp.full_name
...
BEGIN
  ...
  IF NEW.sender_id = v_teacher_id THEN
    ...
    -- Get teacher's name (FIXED: name -> full_name)
  ...
  ELSE
    ...
    -- Get student's name (FIXED: name -> full_name)
    SELECT COALESCE(sp.full_name, p.email, '학생')
    INTO v_sender_name
